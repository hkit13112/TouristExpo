...
export default class AttractionListScreen extends React.Component {
 ...
 /* _Getdata(){
    for (i = 0; i < thamso.length; i++) {
      mydata.push(PlacesDataSource.find(itm => itm.ID ===  this.props.navigation.state.params.thamso[i]))
    }
    return mydata
  }   */

  _goToDetail = id => {
   // console.log('thamso =',this.props.navigation.state.params.thamso)
   //console.log('json places: ',PlacesDataSource)
    ...
    return (            
      <View style={styles.defaultstyle}>
        ...
        <FlatList style={{padding: 5}}
         // data = {this.props.navigation.state.params.thamso}  
          data = {this.props.navigation.state.params.thamso.filter(itm => itm.Catalog.toLowerCase().includes(this.state.catalog.toLowerCase()) && itm.Name.toLowerCase().includes(this.state.Searchkey.toLowerCase()))}        
       ...
       // cai nay dc:   data = {PlacesDataSource.filter(itm => itm.ID ===  this.props.navigation.state.params.thamso[0] || itm.ID ===  this.props.navigation.state.params.thamso[1])}
       //    data = {_Getdata()}   
      // data = {PlacesDataSource.find(this.props.navigation.state.params.thamso[0])}         
